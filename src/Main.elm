...
viewInstructions model =
    if model.showInstructions then
        div [ class "instructions" ]
            ...
            , h1 [] [ text "Setup" ]
            , ul []
                [ li [] [ text "Nonsense requires 4 or more players and 1 phone." ]
                , li [] [ text "Form 2 teams." ]
                , li [] [ text "The game consists of 7 rounds each worth 1 point, the first team to 7 points wins." ]
                ]
            , h1 [] [ text "Gameplay" ]
            , ul []
                [ li [] [ text "At the beginning of each round pick 1 clue giver from each team." ]
                , li [] [ text "Next, the clue givers must agree on a shared secret Word (tap the \"New word\" button to get a Word)." ]
                , li [] [ text "Now, each team takes 45 second turns, back and forth:" ]
                , ul []
                    [ li [] [ text "The current team's clue giver draws a new Rule (tap the \"New rule\" button) and announces the Rule to everyone." ]
                    , li [] [ text "Then, they give clues about the Word to everyone and MUST obey the Rule." ]
                    , li [] [ text "ONLY the CURRENT team can try to guess the Word. There is no limit on guesses." ]
                    , li [] [ text "Clue givers CANNOT tell their team the Word directly, but can confirm or deny a guess." ]
                    , li [] [ text "If the current team cannot guess the Word in time, it becomes the other team's turn." ]
                    ]
                , li [] [ text "The first team to guess the Word wins 1 point and ends the round." ]
                ]
            , h1 [] [ text "Notes" ]
            , ul []
                [ li [] [ text "The losing team should be allowed to start new rounds." ]
                , li [] [ text "Clue givers may skip 1 Rule per round." ]
                , li [] [ text "If a Rule is ever broken or the Word is spoken directly, the team at fault must forfeit 1 point to the other team." ]
                , li [] [ text "Guesses that are proposed outside of a turn's time limit do not count." ]
                , li [] [ text "If both teams are stumped by a Word, upon a majority vote the game can enter Blitz Mode, where:" ]
                , ul []
                    [ li [] [ text "Both clue givers can say anything at any time, except the Word itself." ]
                    , li [] [ text "Both teams may guess at any time." ]
                    , li [] [ text "The first team to guess the Word wins the point." ]
                    ]
                ]
            ]
